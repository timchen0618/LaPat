...
class POSDataUtils(object):
    ...
    Attributes:
        ...
        src_max_len (int): maximum length of input data(including POS)
        max_len (int): maximum length of target data
    ...
    def __init__(self, args):
        ...
        self._dict_path = './5w_pos.json'
        self._test_path = args.test_corpus
        self._train_path = args.corpus
        self._valid_path = args.valid_corpus
        ...
        if self.train:
            ...
            with codecs.open(src_file, encoding="utf8", errors='ignore') as train_f:
                ...
                for line in train_f:
                    ...
                    if data[0] != prev_sent or len(batch['src']) >= self._batch_size:
                        if len(batch['src']) != 0:
                    ...
                    vec1 = self.text2id(text=data[0].strip(),
                                        seq_length=self.src_max_len - self.max_len,
                                        pos=False,
                                        train=not valid
                                       )
                    vec2 = self.text2id(data[3].strip()+' </s>', self.max_len, pos=True, train=not valid)
                    vec3 = self.text2id(data[2].strip()+' </s>', self.max_len, pos=False, train=not valid)
                    ...
                    if vec1 is not None and vec2 is not None and vec3 is not None:
                        ...
                        batch['tgt_mask'].append(subsequent_mask(vec3))
                        batch['pos_y'].append(vec2)
                        batch['y'].append(vec3)
            ...
            for epo in range(1):
                ...
                for line1 in open(filepath):
                    ...
                    vec1 = self.text2id(data[0].strip(), self.src_max_len)
