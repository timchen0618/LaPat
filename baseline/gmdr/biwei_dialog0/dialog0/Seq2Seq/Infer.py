...
class Infer(object):
    ...
    def __init__(self, seq2seq_model, sampler_model,
                 fields,
                 beam_size, n_best=1,
                 max_length=100,
                 global_scorer=None, cuda=False,
                 beam_trace=False, min_length=0):
        self.seq2seq_model = seq2seq_model.eval()
        self.sampler_model = sampler_model.eval()
        self.fields = fields
        self.n_best = n_best
    ...
    def inference_batch(self, batch, tag_input, tag_score):
        ...
        # context, enc_states = self.seq2seq_model.encode(src_inputs, src_lengths)
        # sampler_output = self.sampler_model(src_inputs,src_lengths)
        # tag_log_probs = torch.gather(sampler_output,-1,tag_inputs)
        # if tag_inputs.size(0) < topk_tag:
        #     topk_tag = 1
        # _, selected_tag_pos = tag_log_probs.data.topk(topk_tag,dim=-1)
        # selected_tag = tag_inputs[selected_tag_pos[-1]].unsqueeze(-1)

        # selected_tag_idx = selected_tag.data.tolist()[0][0]
        # tag_hidden = self.seq2seq_model.tag_encode(selected_tag)
        # dec_states = self.seq2seq_model.init_decoder_state((enc_states, tag_hidden))
        ...
        for i in range(self.max_length):
            ...
            # Run one step.
            dec_out, dec_states, attn_scores = self.seq2seq_model.decode(inp, tag_hidden, context, dec_states)
            if not isinstance(dec_states, tuple): # GRU
                dec_states = [ 
                    dec_states
                ]   
            ...
            out = self.seq2seq_model.generator(dec_out).data
            out = unbottle(out)
            # beam x tgt_vocab
            beam_attns = {}
    ...
    def sample_tag_with_kmeans(self, batch, topk_tag=1000, n_cluster=3):
        ...
        # selected_tag = tag_inputs[selected_tag_pos[-1]].unsqueeze(-1)
        tag_hidden = self.seq2seq_model.tag_encode(Variable(selected_tag_pos).unsqueeze(0)).squeeze(0)
        tag_c = self.seq2seq_model.decoder.cal_tag_atten(tag_hidden, context)
