...
class SWDADialogCorpus(object):
    ...
    def __init__(self, corpus_path, max_vocab_cnt=10000, word2vec=None, word2vec_dim=None):
        ...
        # for bug, because I used wrong test corpus 
        self.test_corpus = self.process(data["test_real"])
        self.load_word2vec()
    ...
    def build_vocab(self, max_vocab_cnt):
        ...
        for tokens in self.valid_corpus[self.utt_id]:
            all_words.extend(tokens)
        for tokens in self.test_corpus[self.utt_id]:
            all_words.extend(tokens)
