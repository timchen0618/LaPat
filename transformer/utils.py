    ...
    def data_yielder(self, valid=False):
        ...
        if self.train:
            if self.config['pos_masking']:
                batch = {'src':[], 'tgt':[], 'src_mask':[], 'tgt_mask':[], 'y':[], 'posmask':[]}
            else:
            ...
            with codecs.open(src_file, encoding="utf8", errors='ignore') as train_f:
                ...
                for line in train_f:
                    ...
                    if self.task == 'finetune':
                        ...
                        if self.config['pretrain_utils']:
                            pos = ['<<' + l + '>>' for l in data[3].strip().split()]
                        else:
                            pos = ['<' + l + '>' for l in data[3].strip().split()]

                    if self.config['pos_masking']:
                        if len(pos) > self._max_len:
                            pos = pos[:self._max_len]
                        ### pos_masking ###
                        posmask = torch.zeros((self._max_len, self.vocab_size)).cuda()
                        posmask[:, self.pad] = 1
                        for i, pos_tag in enumerate(pos):
                            posmask[i] = self.pos2mask[pos_tag]
                        ### pos_masking ###
                    ...
                    if self.task == 'pred_pos':
                        vec2 = self.text2id(data[3].strip()+' </s>', self._max_len, pos=True, train=not valid)
                    else:
                        vec2 = self.text2id(data[2].strip()+' </s>', self._max_len, pos=False, train=not valid)

                    if vec1 is not None and vec2 is not None:
                        if self.task == 'finetune' or self.task == 'seq2seq':
                            vec1 = self.addpos2text(pos, vec1, self._src_max_len)
                        if self.task == 'joint_gen':
                            # length = np.count_nonzero((vec2 != self.pad))
                            length = len(data[2].strip().split())+1
                            if length <= self._max_len:
                                # padding = [self.pad] * ((self._max_len - len(pos))*2)                                
                                padding = np.zeros([self._max_len - length]).astype(int)
                                padding.fill(self.pad)
                                pos = self.text2id(data[3].strip() + ' sep', length, pos=True, train=not valid)
                                vec2 = np.concatenate((pos, vec2, padding), axis=0)
                            else:
                                pos = self.text2id(data[3].strip(), self._max_len-1, pos=True, train=not valid)
                                sep = np.array([self.word2id['<sep>']])
                                vec2 = np.concatenate((pos, sep, vec2), axis=0)
                                # assert False
                        ...
                        if self.config['pos_masking']:
                            batch['posmask'].append(posmask.unsqueeze(0))
            ...
            for epo in range(1):
                ...
                for line1 in open(filepath):
                    ...
                    if self.task == 'finetune':
                        ...
                        if self.config['pretrain_utils']:
                            pos = ['<<' + l + '>>' for l in data[2].strip().split()]
                        else:
                            pos = [l for l in data[2].strip().split()]
                    ...
                    if self.config['pos_masking']:
                        if len(pos) > self._max_len:
                            pos = pos[:self._max_len]
                        posmask = torch.zeros((self._max_len+1, self.vocab_size)).cuda()
                        posmask[:, self.pad] = 1
                        for i, pos_tag in enumerate(pos):
                            posmask[i] = self.pos2mask[pos_tag]
                    ### pos_masking ###
