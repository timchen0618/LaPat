...
class SequenceGenerator(nn.Module):
    def __init__(
        ...
        self.model = model
        self.pad = data_utils.pad
        self.unk = data_utils.unk
        self.eos = data_utils.eos
        self.vocab_size = data_utils.vocab_size
        ...
        self.normalize_scores = normalize_scores  #bool
        self.len_penalty = len_penalty            #float (* len_penalty)
        self.unk_penalty = unk_penalty            #float (-=unk_penalty)
        self.retain_dropout = retain_dropout      #bool
        self.temperature = temperature
        self.match_source_len = match_source_len  #bool (=false)
        self.no_repeat_ngram_size = no_repeat_ngram_size
        self.max_len = max_len
        assert temperature > 0, "--temperature must be greater than 0"

        self.search = (
            search.BeamSearch(data_utils) if search_strategy is None else search_strategy
        )
        if not self.retain_dropout:
            self.model.eval()

        print('beam_size: %d max_len: %d min_len: %d'%(self.beam_size, self.max_len, self.min_len))
    ...
    def generate(
        ...
        prefix_tokens: Optional[Tensor] = None,
        bos_token: Optional[int] = None,
        pos_masking: Optional[bool] = False,
    ):
        # net_input = batch["net_input"]
        src_tokens = batch["src"].long()
        ...
        out = self.model.encode(src_tokens, batch['src_mask'])
        encoder_outs = EncoderOut(
            encoder_out=out,  # T x B x C
            encoder_padding_mask=batch['src_mask']
        )
        ...
        if pos_masking:
            batch['posmask'] = batch['posmask'].index_select(0, new_order)

        # initialize buffers
        scores = (
            torch.zeros(bsz * beam_size, max_len + 1).float()
        ...
        for step in range(max_len + 1):  # one extra step for EOS marker
            ...
            if reorder_state is not None:
                if batch_idxs is not None:
                    ...
                    # print('reorder_state', reorder_state)
                ...
                if pos_masking:
                    batch['posmask'] = batch['posmask'].index_select(0, reorder_state)
            ...
            # decode one step
            memory, src_mask = encoder_outs.encoder_out, encoder_outs.encoder_padding_mask
            if not pos_masking:
                lprobs = self.model.decode(memory, src_mask, tokens[:, : step + 1], None, src_tokens)[0][:, -1]
            else:
                # print('tgt', tokens[:, : step + 1].size())
                lprobs = self.model.decode_with_mask(memory, src_mask, tokens[:, : step + 1], None, src_tokens, batch['posmask'][:, step:step+1])[0][:, -1]
                # print('lprobs', lprobs.size())
                # lprobs = lprobs[-1]
                # print('lprobs[-1]', lprobs.size())
            ...
            # print('eos_bbsz_idx', eos_bbsz_idx.size())
            # print('eos_scores', eos_scores.size())
            ...
            # take one step in beam search
            cand_scores, cand_indices, cand_beams = self.search.step(
                step,
                lprobs.view(bsz, -1, self.vocab_size),
                scores.view(bsz, beam_size, -1)[:, :, :step],
            )  #cand_scores -> (bsz, beam_size*2) 
            ...
            # finalize hypotheses that end in eos (eos mask tells us which hyps end in EOS)
            eos_mask = cand_indices.eq(self.eos) & cand_scores.ne(-math.inf)  #(bsz, beam_size*2)
            ...
            # print('new_blacklist', new_blacklist)
            ...
            src_tokens = torch.index_select(
                src_tokens, dim=0, index=active_bbsz_idx
            )
    ...
    def finalize_hypos(
        ...
        for i in range(bbsz_idx.size()[0]):
            ...
            # sent -> true sentence number
            # unfin_idx -> unfinished sentence idx
